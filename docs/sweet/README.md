...
2. 当`点击`操作会打开新标签页时，则下一个步骤做了拟真操作（框架自动切换到新标签页），如步骤6
...
6. 当`点击`产生新标签页（已做了拟真，切换到新标签页），下一个步骤又想回到原标签页，则需在下一个步骤的测试数据中指定标签页名称，如步骤10
...
| 4       |      | 点击 | 搜索按钮          |                  | 当前标签页为：百度 |
| 5       |      | 点击 | 搜索结果#1        |                  | 当前标签页为：百度，点击操作后，浏览器会在新标签页打开豆瓣网站，框架会自动切换到新标签页 |
| 6       |      | 检查 |  title           | 豆瓣              | 当前标签页为：~~豆瓣~~，但没有命名，当切换到其他标签页时，会自动关闭~~豆瓣~~标签页 |
| 7       |      | 点击 | 搜索结果#1        | `#tab=百度`       | 当前标签页为：百度，点击操作后，浏览器会在新标签页打开豆瓣网站，框架会自动切换到新标签页 |
| 8       |      | 检查 |  title           | 豆瓣,`#tab=豆瓣`   |当前标签页为：豆瓣，做了标签页命名，切换标签页不会关闭豆瓣标签页 |
| 9       |      | 点击 | 搜索结果#1        | `#tab=百度`       | 当前标签页为：百度，点击操作胡，框架会自动切换到新标签页~~豆瓣2~~ |
| 10      |      | 点击 | 搜索结果#2        | `#tab=百度`       | 当前标签页为：百度，指定了 #tab，则会切换到百度标签页，再进行操作 |
