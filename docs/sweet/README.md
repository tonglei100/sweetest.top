...
│  ├─baidu.xlsx          # 测试套件：baidu，其中包含了用例、元素列表、测试数据，启动参数
│  └─echo.xlsx           # 测试套件：echo
...
在 Sweet 中，用例、元素列表已经测试数据写在一个 Excel 表格中，这个表格是一个完整的测试套件，请参加示例 echo.xlsx
...
- 在 Sweet 中，我们支持同时测试多个接口或开启多个浏览器，这些对象我们统称为 app，在这里 app 列就是对应的名称，比如测试 postman-echo.com 接口就用 echo 这个名称。
...
- 一般只有 elements 中引用了2次同样的关键字模块，比如2次 http，则 elements 中后面定义的 app，在测试用例中要写 app 的值以示区分。
...
- 在 Sweet 中，web 模块和其他模块一样，都是按需安装、使用，元素定义也直接列在 app 下，标签页切换采用浏览器拟真操作：

1. `打开`操作，需在测试数据中指定当前标签页名称，如步骤1中：`#tab=百度`
2. 当`点击`操作会打开新标签页时，则下一个步骤自动切换到新标签页，如步骤6
3. 步骤6中，没有命名标签页，则当切换标签页时，会自动关闭未命名的标签页
4. 要想新的标签页不关闭，则需要命名，如步骤8
5. 当`点击`产生新标签页，又要强制留在当前标签页，则需在测试数据中指定标签页名称，如步骤10
6. 当有多个标签页（不管有没有命名），且需要手动切换标签页时，请在测试数据中指定标签页名称，如步骤7

| 测试步骤 | 应用 | 操作 | 元素             | 测试数据          |  备注          |
|---------|------|------|-----------------|-------------------|---------------|
| 1       |      | 打开 | http://baidu.com | `#tab=百度`       | 当前标签页为：百度 |
| 2       |      | 检查 | title            | 百度一下，你就知道 | 当前标签页为：百度 |
| 3       |      | 输入 | 搜索框            | 豆瓣             | 当前标签页为：百度 |
| 4       |      | 点击 | 搜索按钮          |                  | 当前标签页为：百度 |
| 5       |      | 点击 | 搜索结果#1        |                  | 当前标签页为：百度，但浏览器会在新标签页打开豆瓣网站 |
| 6       |      | 检查 |  title           | 豆瓣              | 当前标签页为：~~豆瓣~~，但没有命名，当切换到其他标签页时，会自动关闭~~豆瓣~~标签页 |
| 7       |      | 点击 | 搜索结果#1        | `#tab=百度`       | 当前标签页为：百度 |
| 8       |      | 检查 |  title           | 豆瓣,`#tab=豆瓣`   |当前标签页为：豆瓣，切换标签页不会关闭豆瓣标签页 |
| 9       |      | 点击 | 搜索结果#1        | `#tab=百度`       | 当前标签页为：百度 |
| 10      |      | 点击 | 搜索结果#2        | `#tab=百度`       | 当前标签页为：百度 |
