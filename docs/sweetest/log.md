...
测试组合：即一组测试用例。测试用例 id 中，如有 # 号则该用例为测试组合中的用例，第一个 # 号前的字符为组合名称
...
如上3个用例为一个 buy 用例组合，那么我们可以用如下方式循环调用

| 用例编号   | 用例标题  | 前置条件  | 测试步骤 | 操作  | 页面      | 元素   | 测试数据 |
| --------- | -------- | --------- | ------- |-------| -------- | ----- | ---------|
| buy       | 网购     |           | 1       | `执行`   | `用例组合` | `buy`*2 |    |

我们可以在测试数据中给变量赋值（和执行用例片段的循环赋值类似）
用例组合执行结果将按顺序添加在测试套件结尾
