...
注意：
  1. 测试数据中，多个 `k=v` 用英文逗号(`,`)分隔
  2. 如果 `v` 中有英文逗号(`,`)，要用反斜杠(`\`)转义
  3. 如果  `v` 中有很多英文逗号(`,`)，为了方便书写，框架也支持以双英文逗号(`,,`)分割，那么  `v` 中的单英文逗号(,)的内容无需转义，例如：

| 操作  | 页面   | 元素  | 测试数据    |
| --- | ------ | ------- | ----------- |
| GET | Echo   | get请求  | `params={'foo1': 'bar1', 'foo2': 'bar2'},,cookies={'user': 'test'}` |

> 英文逗号(`,`) 和双英文逗号(`,,`)可以混用，但一个单元格内只能使用一种格式。
...
在所有的 Web 操作步骤中，都可以进行截图输出和截图比较，且截图动作是在本步骤操作完成之后进行。

#### 截图输出

在输出数据中,将截图赋值给变量。

`#ScreenShot`表示截屏，`home_png` 保存了截图的路径。
...
`#ElementShot`表示元素截图，`search_png`保存了截图的路径。

| 操作  | 页面     | 元素    | 测试数据 | 输出数据                   |
| ----- | ------- | ------- | ---------| --------------------------|
| 点击  | 百度搜索 | 搜索按钮 |          | `search_png=#ElementShot` |

> 注：可以在启动脚本中设置全局截图，则每个 Web 操作步骤都会自动截屏保存。

#### 截图比较

在测试数据中，对截图进行比较。截图比较有2中模式：

##### 动态比较

将本步骤的截图和运行时前面步骤的截图输出进行比较，如下：

`#ScreenShot`表示本步骤截屏，`<home_png>` 为前面步骤输出的截图。

| 操作 | 页面     | 元素      | 测试数据                                    | 
| ---- | ------- | -------- | -------------------------------------------- | 
| 检查 | 百度搜索 | 页面title  | 百度一下，你就知道, `#ScreenShot=<home_png>` | 

`#ElementShot`表示本步骤元素截图，`<search_png>`为前面步骤输出的截图。
...
`#ScreenShot`表示本步骤截屏，`home.png` 为预期的截图文件名。

| 操作 | 页面     | 元素      | 测试数据                                    | 
| ---- | ------- | -------- | -------------------------------------------- | 
| 检查 | 百度搜索 | 页面title  | 百度一下，你就知道, `#ScreenShot=home.png` | 

`#ElementShot`表示本步骤元素截图，`search.png`为预期的截图文件名。

| 操作  | 页面     | 元素    | 测试数据 | 输出数据                   |
| ----- | ------- | ------- | ---------| --------------------------|
| 点击  | 百度搜索 | 搜索按钮 |          | `search_png=search.png` |

几点说明：

- 预期的截图文件名必须以 .png 结尾
- 预先保存的预期截图必须放在 `snapshot\expected\<plan_name>` 目录
- 但实际上我们不需要手动去截图保存，框架已经自动识别是否有预期截图存在，如果不存在，则当前运行把截图保存为预期图片，且不做比较
...
如果参数是字符，需要带引号，如：`<u.td(t='1')>`

如果参数是已有变量，直接写即可，如，已知 t=3，写法为：`<u.td(t=t)>`
