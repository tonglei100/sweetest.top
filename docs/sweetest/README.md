...
自动义函数在生成示例目录的 lib 下面。

#### 定义

在 lib/c.py or lib/u.py 中添加自己定义的函数。也可以自己创建 .py 文件，但需要添加文件名到在 lib/__init__.py 中。

lib/http_handle.py 是 Http 测试的预处理函数()不可以删除或修改名称)。如果接口是需要签名加密处理的，需要自行在此函数中处理。
...
如果参数是已有变量，直接写即可，如，已知 t=3，那么，`<u.td(t=t)>`
